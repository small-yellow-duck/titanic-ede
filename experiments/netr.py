...
class Decoder(nn.Module):
    def __init__(self, input_dict, maxlens, dim=24, recurrent_hidden_size=8):
        ...
        self.main_layers = {}
        self.main_layers[0] = nn.Sequential(nn.utils.weight_norm(nn.Conv2d(int(self.dim / 1), int(self.hidden_dim / 1), 1, dilation=1, stride=1)))
        self.main_layers[1] = nn.Sequential(nn.PReLU())
        self.main_layers[2] = nn.Sequential(nn.utils.weight_norm(nn.Conv2d(int(self.dim / 1)+self.hidden_dim, int(self.hidden_dim / 1), 1, dilation=1, stride=1)))
        self.main_layers[3] = nn.Sequential(nn.PReLU())
        self.main_layers[4] = nn.Sequential(nn.utils.weight_norm(nn.Conv2d(int(self.dim / 1)+int(self.hidden_dim / 1), int(self.hidden_dim / 1), 1, dilation=1, stride=1)))
        self.main_layers[5] = nn.Sequential(nn.PReLU())

        for layer_num, layer in self.main_layers.items():
            self.add_module('layer_'+str(layer_num), layer)

    def main(self, latent_vec):
        latent_vec = latent_vec.view(latent_vec.size(0), -1, 1, 1)
        x = self.main_layers[0](latent_vec)
        x0 = self.main_layers[1](x)
        x = torch.cat((latent_vec, x0), 1)
        x = self.main_layers[2](x)
        x = self.main_layers[3](x)
        x = torch.cat((latent_vec, x), 1)
        x = self.main_layers[4](x)
        x = self.main_layers[5](x)
        return x
...
class Encoder(nn.Module):
    ...
    def __init__(self, input_dict, dim=24, recurrent_hidden_size=8, sigmoidout=False):
        ...
        self.main_layers = {}
        self.main_layers[0] = nn.Sequential(
        ...
        self.main_layers[1] = nn.Sequential(nn.PReLU())
        self.main_layers[2] = nn.Sequential(nn.utils.weight_norm(nn.Conv2d(self.input_size+int(self.hidden_dim / 1), int(self.hidden_dim / 1), 1, dilation=1, stride=1)))
        self.main_layers[3] = nn.Sequential(nn.PReLU())
        self.main_layers[4] = nn.Sequential(nn.utils.weight_norm(nn.Conv2d(self.input_size+int(self.hidden_dim / 1), int(self.hidden_dim / 1), 1, dilation=1, stride=1)))
        self.main_layers[5] = nn.Sequential(nn.PReLU())

        for layer_num, layer in self.main_layers.items():
            self.add_module('layer_'+str(layer_num), layer)

    def main(self, input_vec):
        input_vec = input_vec.view(input_vec.size(0), -1, 1, 1)
        x = self.main_layers[0](input_vec)
        x = self.main_layers[1](x)
        x = torch.cat((input_vec, x), 1)
        x = self.main_layers[2](x)
        x = self.main_layers[3](x)
        x = torch.cat((input_vec, x), 1)
        x = self.main_layers[4](x)
