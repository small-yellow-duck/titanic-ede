...
def train(rawdata, charcounts, maxlens, unique_onehotvals):
    ...
    for fold in range(1):
        ...
        for it in range(1000000):
            ...
            enc_loss = logloss(torch.mean(torch.pow(mu[::2]-mu[1::2], 2), 1), are_same)
            #enc_loss += 0.5*contrastivec(mu2[::2], mu2[1::2], are_same)
            #enc_loss += 0.5 * contrastivec(mu[::2], mu2[1::2], are_same)
            enc_loss += 1.0*logloss(torch.mean(torch.pow(mu-mu2,2), 1), torch.ones(mb_size).cuda())
            enc_loss += 2.0*logloss(torch.mean(torch.pow(mu-mu2d, 2), 1), torch.zeros(mb_size).cuda())
